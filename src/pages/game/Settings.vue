<template>
  <div class="settings">
    <setting-group>
      <setting-item title="Instance Name" description="The name of this game instance." icon="signature">
      ...
      <setting-item v-if="props.instance.config.name === 'Latest Release'"
        :title="$t('settings.accessibility.hideLatestRelease')"
        :description="$t('settings.accessibility.hideLatestReleaseDesc')" icon="eye-slash">
        <toggle-switch v-model="config.accessibility.hide_latest_release"></toggle-switch>
      </setting-item>
      <setting-item v-if="props.instance.config.name === 'Latest Snapshot'"
        :title="$t('settings.accessibility.hideLatestSnapshot')"
        :description="$t('settings.accessibility.hideLatestSnapshotDesc')" icon="eye-slash">
    ...
    <setting-group :title="$t('settings.game.launchOptions')">
      ...
      <setting-item :title="$t('settings.game.worldName')" :description="$t('settings.game.worldNameDesc')"
        icon="floppy-disk">
        <TextInputBox width="300px" :placeholder="$t('settings.game.worldNamePlaceholder')"></TextInputBox>
      </setting-item>
      ...
      <setting-item :title="$t('settings.game.autoRefreshAccount')"
        :description="$t('settings.game.autoRefreshAccountDesc')" icon="user-check">
        <toggle-switch v-model="config.launch.skip_refresh_account"></toggle-switch>
      </setting-item>
      <setting-item :title="$t('settings.game.autoCompleteGameFiles')"
        :description="$t('settings.game.autoCompleteGameFilesDesc')" icon="file-check">
        <toggle-switch v-model="config.launch.skip_check_files"></toggle-switch>
      </setting-item>
      <setting-item :title="$t('settings.game.demo')" :description="$t('settings.game.demoDesc')" icon="lock">
    ...
    <setting-group :title="$t('settings.advance.launchArgs')">
      ...
      <setting-item :title="$t('settings.advance.extraJVMArgs')" :description="$t('settings.advance.extraJVMArgsDesc')">
      ...
      <setting-item title="Open Log Viewer" description="The Description of Open Log Viewer " icon="scroll"
        :clickAble="true" @click="logViewerOpen = true">
        <i class="chevron-right" style="margin-right: 10px"></i>
      </setting-item>
    </setting-group>
    <setting-group title="Danger Zone" :danger="true">
      <setting-item title="Delete This Instance"
        description="Once you delete a instance, there is no going back. Please be certain." icon="trash-can"
        :clickAble="true" @click="confirmDeleteInstanceVisible = true" :disabled="props.instance.config.name === 'Latest Release' ||
          props.instance.config.name === 'Latest Snapshot'
          ">
        <i class="chevron-right" style="margin-right: 10px"></i>
      </setting-item>
      <setting-item title="Reset This Instance"
        description="Clear all data in this instance, including worlds, packages, and modules" icon="arrow-rotate-right"
        :clickAble="true">
        <i class="chevron-right" style="margin-right: 10px"></i>
      </setting-item>
    </setting-group>
    <confirm-delete-instance :visible="confirmDeleteInstanceVisible" :instance="props.instance"
      @close="confirmDeleteInstanceVisible = false" @deleted="
        confirmDeleteInstanceVisible = false;
      $emit('update-instance-list');
      "></confirm-delete-instance>
    <LogViewer :instance-name="props.instance.config.name" :visible="logViewerOpen" @close="logViewerOpen = false">
    </LogViewer>
  </div>
</template>
