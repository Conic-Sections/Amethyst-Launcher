<template>
  <dialog-vue :visible="show" width="460" height="480">
    <div class="main">
      ...
      <TransitionGroup>
        <list-item v-for="(version, index) in versions" :key="index" :title="`Minecraft ${version.id}`" logo="1" :click-able="true"
          @click="$emit('select', version.id)" :buttons="['circle-info', 'floppy-disk']" :description="parseTime(version.releaseTime)">
          <template #icon>
            <img v-if="version.type == `release`" style="width: 100%; height: 100%;"
              src="@/assets/images/Grass_Block.webp" alt="">
            <img v-else-if="version.type == `snapshot`" style="width: 100%; height: 100%;"
              src="@/assets/images/Command_Block.webp" alt="">
          ...
          <template #subtitle v-if="version.type == 'release'">
            <tag text="正式版" :color="['74', '194', '107']" :background="true" :border="true"></tag>
          </template>
...
invoke('get_minecraft_version_list').then((res: any) => {
  if (res != null) {
