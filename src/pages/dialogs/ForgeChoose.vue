<template>
  <dialog-vue :visible="show" width="460" height="480">
    <div class="main">
      ...
      <TransitionGroup>
        <list-item title="不选择 Forge" :click-able="true" @click="$emit('select', '')" :key="0"></list-item>
        <list-item v-for="(version, index) in versions" :key="index + 1" :title="version.version" logo="1"
          :click-able="true" @click="$emit('select', version.version)" :buttons="['circle-info', 'floppy-disk']"
